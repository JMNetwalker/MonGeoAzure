...
try
 {
   ...
   for  ($i=0; $i -lt $QueriesDBUser.length; $i++) {
        Write-Host -ForegroundColor Green "Starting:" ($QueriesDBUser[$i])
          Invoke-Sqlcmd -ServerInstance $server -Database $databaseDBUser -Query $QueriesDBUser[$i] -Username $user -Password $password -ConnectionTimeout 60 -QueryTimeout 60 | Select-Object *  | ConvertTo-Json  | Out-File -filePath ($folder+$fecha+$filesDbUser[$i]+".json") -Encoding "UTF8"
        Write-Host -ForegroundColor Green "Completed:" ($QueriesDBUser[$i])
     }
   for  ($i=0; $i -lt $QueriesDBMaster.length; $i++) {
        Write-Host -ForegroundColor Green "Starting:" ($QueriesDBMaster[$i])
          Invoke-Sqlcmd -ServerInstance $server -Database $databaseDBMaster -Query $QueriesDBMaster[$i] -Username $user -Password $password -ConnectionTimeout 60 -QueryTimeout 60 | Select-Object *  | ConvertTo-Json  | Out-File -filePath ($folder+$fecha+$filesDbMaster[$i]+".json") -Encoding "UTF8"
        Write-Host -ForegroundColor Green "Completed:" ($QueriesDBMaster[$i])
     }
  }
catch
  {
